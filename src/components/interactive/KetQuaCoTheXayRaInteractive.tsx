...
const generatePractice = (): Problem => {
  ...
  if (type === 0) {
    ...
    return { text: `Tung ${n} đồng xu. Tổng số kết quả có thể xảy ra = ?`, answer: `${total}`, hint: `2^${n} = ${total}` };
  } else if (type === 1) {
    const faces = randInt(2, 4);
    const total = faces * faces;
    return { text: `Tung ${faces} con xúc xắc. Tổng số kết quả = ?`, answer: `${total}`, hint: `${faces} × ${faces} = ${total}` };
  } else {
    const n = randInt(3, 8);
    const pass = randInt(1, n - 1);
    return { text: `${n} quả bóng, ${pass} đỏ, còn lại xanh. Lấy 1 quả, xác suất lấy được đỏ = ?`, answer: `${pass}/${n}`, hint: `P(đỏ) = ${pass}/${n}` };
  }
};

const generateQuick = (): Problem => {
  const n = randInt(2, 5);
  return { text: `Tung ${n} đồng xu, mỗi đồng có 2 mặt. Tổng kết quả = ?`, answer: `${Math.pow(2, n)}`, hint: `2^${n} = ${Math.pow(2, n)}` };
};

const generateFindX = (): Problem => {
  const total = randInt(6, 12);
  const red = randInt(2, total - 2);
  return { text: `${total} bóng, ${red} đỏ. P(đỏ) = ${red}/${total}. Tổng số bóng = x.`, answer: `${total}`, hint: `x = ${total}` };
};

const generators = [generatePractice, generateQuick, generateFindX];
const tabLabels = ['Luyện tập', 'Nhanh', 'Tìm x'];

export default function KetQuaCoTheXayRaInteractive() {
  ...
  return (
    <div style={{ margin: '2rem 0', padding: '1.5rem', background: 'white', borderRadius: 16, border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
      <h3 style={{ margin: '0 0 1rem', fontSize: '1.25rem', color: '#1e293b' }}>🎲 Kết quả có thể xảy ra — Luyện tập tương tác</h3>
