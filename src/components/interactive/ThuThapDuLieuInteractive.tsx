...
const generatePractice = (): Problem => {
  ...
  if (type === 0) {
    ...
    const cat = randInt(5, total - 10);
    const dog = randInt(5, total - cat - 5);
    return { text: `Lớp có ${total} HS: ${cat} bạn thích mèo, ${dog} bạn thích chó, còn lại thích cá. Bao nhiêu bạn thích cá?`, answer: `${total - cat - dog}`, hint: `Cá = ${total} - ${cat} - ${dog} = ${total - cat - dog}` };
  } else if (type === 1) {
    const n = randInt(3, 8);
    return { text: `Để thu thập dữ liệu về màu xe của học sinh, dùng biểu đồ nào phù hợp nhất? (1=Biểu đồ cột, 2=Biểu đồ tròn, 3=Biểu đồ tranh)`, answer: '1', hint: `Biểu đồ cột thể hiện số lượng từng loại.` };
  } else {
    const total = randInt(40, 80);
    const pass = randInt(20, total - 5);
    const rate = Math.round(pass / total * 100);
    return { text: `${total} HS thi, ${pass} HS đậu. Tỉ lệ đậu là bao nhiêu phần trăm?`, answer: `${rate}%`, hint: `Tỉ lệ = ${pass}/${total} × 100% = ${rate}%` };
  }
};

const generateQuick = (): Problem => {
  const total = randInt(30, 60);
  const a = randInt(5, total - 10);
  const b = randInt(5, total - a - 5);
  return { text: `Tổng ${total}, có ${a} và ${b}. Còn lại = ?`, answer: `${total - a - b}`, hint: `${total} - ${a} - ${b} = ${total - a - b}` };
};
...
const generateFindX = (): Problem => {
  ...
  const pass = randInt(20, total - 5);
  const rate = Math.round(pass / total * 100);
  return { text: `${total} HS thi, tỉ lệ đậu x%. Số HS đậu = ${pass}. Tìm x.`, answer: `${rate}`, hint: `${pass}/${total} × 100% = ${rate}%` };
};
...
export default function ThuThapDuLieuInteractive() {
  ...
  return (
    <div style={{ margin: '2rem 0', padding: '1.5rem', background: 'white', borderRadius: 16, border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
      <h3 style={{ margin: '0 0 1rem', fontSize: '1.25rem', color: '#1e293b' }}>📊 Thu thập dữ liệu — Luyện tập tương tác</h3>
