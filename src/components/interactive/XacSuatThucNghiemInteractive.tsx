...
const generatePractice = (): Problem => {
  ...
  if (type === 0) {
    ...
    return { text: `Hộp có ${total} bóng: ${red} đỏ, ${total - red} xanh. Lấy 1 quả, P(đỏ) = ?`, answer: `${red}/${total}`, hint: `P(đỏ) = số bóng đỏ / tổng = ${red}/${total}` };
  } else if (type === 1) {
    const total = randInt(10, 30);
    const red = randInt(2, total - 2);
    const freq = randInt(5, 20);
    const observed = randInt(1, freq);
    const rate = Math.round(observed / freq * 100);
    return { text: `Tung xúc xắc ${freq} lần, mặt 6 xuất hiện ${observed} lần. Tần suất = ?%`, answer: `${rate}%`, hint: `${observed}/${freq} × 100% = ${rate}%` };
  } else {
    const n = randInt(2, 6);
    return { text: `Tung ${n} đồng xu. Xác suất tất cả đều mặt_heads = ?`, answer: `1/${Math.pow(2, n)}`, hint: `Mỗi đồng P(heads) = 1/2. Tổng P = (1/2)^${n} = 1/${Math.pow(2, n)}` };
  }
};
...
const generateQuick = (): Problem => {
  const total = randInt(10, 30);
  const red = randInt(2, total - 2);
  return { text: `${total} bóng, ${red} đỏ. P(đỏ) = ?`, answer: `${red}/${total}`, hint: `${red}/${total}` };
};

const generateFindX = (): Problem => {
  const total = randInt(10, 30);
  const red = randInt(2, total - 2);
  return { text: `P(đỏ) = ${red}/${total}. Tổng bóng = x, bóng đỏ = ${red}. x = ?`, answer: `${total}`, hint: `x = ${total}` };
};

const generators = [generatePractice, generateQuick, generateFindX];
const tabLabels = ['Luyện tập', 'Nhanh', 'Tìm x'];

export default function XacSuatThucNghiemInteractive() {
  ...
  return (
    <div style={{ margin: '2rem 0', padding: '1.5rem', background: 'white', borderRadius: 16, border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
      <h3 style={{ margin: '0 0 1rem', fontSize: '1.25rem', color: '#1e293b' }}>🎲 Xác suất thực nghiệm — Luyện tập tương tác</h3>
