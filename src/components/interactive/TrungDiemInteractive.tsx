...
const generatePractice = (): Problem => {
  ...
  if (type === 0) {
    const ab = randInt(6, 40);
    return { text: `Cho AB = ${ab}cm, M là trung điểm. Tính AM.`, answer: `${ab / 2}`, hint: `AM = AB/2 = ${ab}/2 = ${ab / 2}` };
  } else if (type === 1) {
    const am = randInt(3, 20);
    return { text: `M là trung điểm AB, AM = ${am}cm. Tính AB.`, answer: `${am * 2}`, hint: `AB = 2 × AM = 2 × ${am} = ${am * 2}` };
  } else {
    const ab = randInt(6, 40);
    return { text: `AB = ${ab}cm, M là trung điểm. Tính AM + MB.`, answer: `${ab}`, hint: `AM + MB = AB = ${ab}cm (vì M nằm giữa A và B)` };
  }
};

const generateQuick = (): Problem => {
  const am = randInt(3, 15);
  return { text: `M là trung điểm AB, AM = ${am}cm. MB = ?`, answer: `${am}`, hint: `M là trung điểm → AM = MB = ${am}` };
};

const generateFindX = (): Problem => {
  const ab = randInt(10, 40);
  return { text: `AB = ${ab}cm, M là trung điểm. AM = x cm. Tìm x.`, answer: `${ab / 2}`, hint: `x = AB/2 = ${ab}/2 = ${ab / 2}` };
};

const generators = [generatePractice, generateQuick, generateFindX];
const tabLabels = ['Luyện tập', 'Nhanh', 'Tìm x'];

export default function TrungDiemInteractive() {
  ...
  return (
    <div style={{ margin: '2rem 0', padding: '1.5rem', background: 'white', borderRadius: 16, border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
      <h3 style={{ margin: '0 0 1rem', fontSize: '1.25rem', color: '#1e293b' }}>📍 Trung điểm — Luyện tập tương tác</h3>
